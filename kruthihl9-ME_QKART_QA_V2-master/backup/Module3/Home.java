...
public class Home {
    ...
    public Boolean PerformLogout() throws InterruptedException {
        try {
            ...
            Thread.sleep(3000);// SLEEP_STMT_10
    ...
    public Boolean searchForProduct(String product) {
        try {
            ...
            // box
            WebElement sesrch_box = driver.findElement(By.name("search"));
            sesrch_box.clear();
            sesrch_box.sendKeys(product);
            ...
            // wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//p[contains(@class,
            // 'MuiTypography-root MuiTypography-body1 css-yg30e6')]")));
            return true;
        } catch (Exception e) {
    ...
    public Boolean isNoResultFound() {
        ...
        try {
            ...
            WebElement noResultFound = driver.findElement(By.tagName("h4"));
            if (noResultFound.isDisplayed()) {
                String actualResult = noResultFound.getText();
                if (actualResult.equals(" No products found ")) {
                    status = true;
                }
            }
            return status;
    ...
    public Boolean addProductToCart(String productName) {
        try {
            ...
            for (WebElement productsfound : listProduct) {
                String productNameText = productsfound.getText();
    ...
    public Boolean changeProductQuantityinCart(String productName, int quantity) {
        try {
            ...
            WebElement quantityInput =
                    driver.findElement(By.xpath("//div[@class='MuiBox-root css-69i1ev']/div/div"));
            int currquantityInputValue = Integer.parseInt(quantityInput.getText());

            for (WebElement productelementNamelist : productElement) {
                String productelementName = productelementNamelist.getText();
                if (productelementName.equalsIgnoreCase(productName)) {
                    int quantityDifference = quantity - currquantityInputValue;
                    if (quantityDifference > 0) {
                        for (int i = 0; i < quantityDifference; i++) {
                            driver.findElement(By
                                    .xpath("//div[@class='MuiBox-root css-69i1ev']/div/button[2]"))
                                    .click();

                            Thread.sleep(5000);
                            // WebDriverWait wait = new WebDriverWait(driver, 5);  
                            // wait.until(ExpectedConditions.textToBePresentInElement(quantityInput,
                            //         String.valueOf(currquantityInputValue + 1)));
                        }
                    } else {
                        for (int i = 0; i > quantityDifference; i--) {
                            driver.findElement(By
                                    .xpath("//div[@class='MuiBox-root css-69i1ev']/div/button[1]"))
                                    .click();
