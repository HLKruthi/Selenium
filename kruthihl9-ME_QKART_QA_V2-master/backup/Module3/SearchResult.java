...
public class SearchResult {
    ...
    public String getTitleofResult() {
        ...
        WebElement titleofResult = parentElement.findElement(
                By.xpath(".//p[@class='MuiTypography-root MuiTypography-body1 css-yg30e6']"));
        titleOfSearchResult = titleofResult.getText();
    ...
    public Boolean openSizechart() {
        try {
            ...
            WebElement openSize =
                    parentElement.findElement(By.xpath("(.//button[@type='button'])[1]"));
            openSize.click();
            Thread.sleep(2000);
            return true;
        } catch (Exception e) {
    ...
    public Boolean verifySizeChartExists() {
        ...
        try {
             ...
             */
            WebElement sizeChartExists =
                    parentElement.findElement(By.xpath(".//button[text()='Size chart']"));
            if (sizeChartExists.isDisplayed()) {
                String actualSizeChart = sizeChartExists.getText();
                if (actualSizeChart.equalsIgnoreCase("SIZE CHART")) {
                    status = true;
                }
            }
            return status;
    ...
    public Boolean validateSizeChartContents(List<String> expectedTableHeaders,
        ...
        try {
            ...
            for (int i = 0; i < expectedTableHeaders.size(); i++) {
                String expectedTableres = expectedTableHeaders.get(i);
                int col = i + 1;
                String tableHeaderXpath = "//table/thead/tr/th[" + col + "]";
                WebElement tableHeader = driver.findElement(By.xpath(tableHeaderXpath));
                String actualHeader = tableHeader.getText();
                if (!actualHeader.equalsIgnoreCase(expectedTableres)) {
                    status = false;
                }
            ...
            for(int i = 0; i < expectedTableBody.size(); i++){
                ...
                for(int j = 0 ; j <  tableRow.size() ; j++){
                    String expectedTableBodyValue = tableRow.get(j);
                    int rows = i+1;
                    int cols = j+1;
                    String tableBodyXpath = "//table/tbody/tr["+rows+"]/td["+cols+"]";
                    WebElement tbody = driver.findElement(By.xpath(tableBodyXpath));
                    String actualBodyValue = tbody.getText();
                    if(!actualBodyValue.equalsIgnoreCase(expectedTableBodyValue)){
                        status = false;
                    }
    ...
    public Boolean verifyExistenceofSizeDropdown(WebDriver driver) {
        ...
        try {
            ...
            WebElement existenceOfSizeDropdown =
                    driver.findElement(By.xpath("//select[@name='age']"));
            if (existenceOfSizeDropdown.isDisplayed()) {
                status = true;
            }
